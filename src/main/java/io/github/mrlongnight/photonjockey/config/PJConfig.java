 ...
 */
public class PJConfig implements Config {

    private static final String LIST_SPACER = "■";

    private static final Logger logger = LoggerFactory.getLogger(PJConfig.class);
    ...
    public PJConfig() {
        ...
        // migrate from old preferences package structure
        try {
            ...
            if (!preferences.getBoolean(MIGRATION_FLAG_KEY, false)
                ...
                if (oldPrefs.keys().length > 0) {
                    ...
                    Arrays.stream(oldPrefs.keys())
                            .forEach((key) -> preferences.put(key, oldPrefs.get(key, "")));
                    logger.info("Migration successful.");
                } else {
                    logger.info("No old settings found at {}. Skipping migration.", OLD_PACKAGE_PATH);
                }

                preferences.putBoolean(MIGRATION_FLAG_KEY, true);
                preferences.flush();
            }
